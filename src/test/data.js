...
m.OZ = {
	...
	speedSet: function(f2, f3, id, value, basetime, baseSpeed) {
		...
		cs.p("speedSet,flag1=170,flag2=", f2, ",flag3=", f3, ",id=", id,
			",speed=", speed)
	},
...
MUSIC.module.webPlayer.interFace = function() {
		...
		jsloader.onload = function() {
			try {
				...
				if (/win/.test(navigator.platform.toLowerCase()) &&
					((!!ua.firefox || !!ua.ie) && typeof g_dialog != "undefined")) g_dialog.show({
					mode: "common",
	...
	function startPlayer() {
		if (!playerList ||
			playerList.getCount() <= 0) return false;
	...
	function setCurrentPlayerSource(args) {
		!!VQQPlayer &&
			VQQPlayer.setCurrentPlayerSource(args)
	}
			...
			expPlayUL = function() {
				var d = top.g_JData["express"],
					...
					ipv6PlayUL = function() {
						...
						} else m.isIpv6 = data.ipinfo == "ipv6"; if (m.useIpv6 && (m.isIpv6 && (MUSIC.lang.isHashMap(d) && d.code == 0))) {
							m.expType = 9;
							url = "http://ipv6.base.music.qq.com/" + mid + ".mp3?vkey=" + d.key + "&guid=" +
								m_r_GetRUin()
						}
				...
				try {
					...
					} else callback(songobj.playUrl = url); if (!m.tExp) {
						m.tExp = 1;
						setTimeout(function() {
							m.getExp = 0;
							m.tExp = 0
...
MUSIC.module.webPlayer.playerList = function() {
	...
	function setPlayerList(arr, mode) {
		...
		for (var i = 0, len = arr.length; i < len; i++)
			if (typeof arr[i] == "object") mpList.push(arr[i]);
		mPostion = -1;
	...
	function addSongList(list) {
		for (var i = 0, len = list.length; i < len; i++)
			if (typeof list[i] == "object") mpList.push(list[i])
	}

	function delSong(pos) {
		if (pos >= 0 && pos < mpList.length) mpList.splice(pos, 1);
		if (pos < mPostion) mPostion--;
		if (mPostion >= mpList.length) mPostion = mpList.length -
			1;
	...
	function clearPlayerList() {
		for (var i = 0, len = mpList.length; i < len; i++) delete mpList[i];
		mpList = [];
		mPostion = -1
	}
	return {
...
MUSIC.module.webPlayer.qqPlayer = function(fromTag) {
	...
	function insertQQPlayer(args) {
		...
		if (isIE) {
			...
			if (QzonePlayer.GetVersion(5) >= "3.2") {
				QzonePlayer.setAttribute("P2PUDPServ_IP",
					"pdlmusic.p2p.qq.com");
	...
	function initialize() {
		try {
			...
			EventPlayer(mPlayerName,
				"OnDnldProgress", g_playerCallback.OnDownloadProgress);
...
MUSIC.module.webPlayer.eventCallback = function() {
	...
	function OnPlayError(lErrCode, sErrDesc) {
		...
		cs.p("g_playerCallback.OnPlayError 2,_obj:",
			_obj, "index:", index);
...
function statImgClean() {
	for (var i = 0, l = window.tmpMusicStat.length; i < l; i++)
		if (!!window.tmpMusicStat[i] && !!window.tmpMusicStat[i].ready) delete window.tmpMusicStat[i]
}
MUSIC.module.webPlayer.stat = function() {
	...
	function submit(noTimeout) {
		...
		if (count > 0) {
			var statUrl = "http://pt.music.qq.com/fcgi-bin/cgi_music_webreport.fcg?Count=" + count + "&Fqq=" + g_user.getUin() + "&Fguid=" +
				_getGuid() + "&Ffromtag1=" + _fromtag1 + "&Ffromtag2=" + fromtag2.join(",") + "&Fsong_id=" + id.join(",") + "&Fplay_time=" + playtime.join(",") + "&Fstart_time=" + starttime.join(",") + "&Ftype=" + type.join(",") + "&Fversion=" + _playerType;
			if (noTimeout) {
				_img = new Image;
				_img.src = statUrl
...
MUSIC.scrollbar = function() {
	...
	function init(options) {
		...
		if (newheight > barpsz[1] - _options.bartop * 2) {
			...
			E.addEvent(MUSIC.dom.get("contentFrame"), "mouseout", function() {
...
MUSIC.channel.top_player = function() {
	var _curTab = "playlist",
		...
		_playListEmpty = true;
	_playlistBar = {}, _favlistBar = {}, $ = MUSIC, $C = $.css, $D = $.dom, $E = $.event;
	...
	var _localSong = {
			...
			try {
				g_storage.get("qqmusic_playlist",
					function(data) {
						...
						if (!!data)
							if (window.JSON && window.JSON.parse) top["qqmusic_playlist"] = JSON.parse(data);
							else eval('top["qqmusic_playlist"] = ' + data);
						_this._list = [];
						MUSIC.object.each(top["qqmusic_playlist"], function(music) {
		...
		save: function() {
			var list = [],
				count = 0,
				tmplist = [];
			for (var i = 0, len = this._list.length; i <
				len; i++) {
				...
				var _fun = removeEvent ? $E.removeEvent : $E.addEvent,
					_listClick = function(e) {
						var _target = $E.getTarget(e),
							...
							_liElement = function() {
								var _cache = _target;
								while (_cache && !/li/ig.test(_cache.tagName)) _cache =
									_cache.parentNode;
								return _cache
							}();
						if (/btn_like/ig.test(_className)) {
							listFrame.getSelectSongInfo(_liElement, function(songInfo) {
						...
						if (/btn_share/ig.test(_className)) {
							listFrame.getSelectSongInfo(_liElement, function(songInfo) {
								_playerOp.share(songInfo, _liElement)
							});
							return
						}
		...
		return {
			...
			setPlayList: function(dirid, callback) {
				if (dirid == _curPlayDirid) callback();
					...
					var uin = g_user.getUin();
					g_mymusicAlbumMod.getDetail(uin, dirid, function(data) {
			...
			getSelectSongInfo: function(obj, callback) {
				...
				if (pos >= 0 && dirid >= 0)
					if (dirid == 0) _localSong.get(function(list) {
						if (pos < list.length) {
							var music = list[pos];
							music.dirid = dirid;
							callback(music)
			...
			getSelectPos: function(obj) {
				...
				for (var i = _localSong._list.length - 1; i >= 0; i--)
					if (_mid == _localSong._list[i].playid) return i;
				return -1
			},
			renderSongList: function(list) {
				...
				if (uin < 10001) _list.innerHTML = "<ul>" + listFrame.getSongListHtml(list) + "</ul>";
					...
					g_mymusicAlbumMod.Song.getMyfavMap(uin, function(mapMyfav) {
			...
			addSongHtml: function(list) {
				var _es =
					$D.get("divsonglist").getElementsByTagName("ul");
				if (_es.length > 0) {
					var uin = g_user.getUin();
					if (uin < 10001) $D.insertAdjacent(_es[0], 2, listFrame.getSongListHtml(list));
					else g_mymusicAlbumMod.Song.getMyfavMap(uin, function(mapMyfav) {
						$D.insertAdjacent(_es[0], 2, listFrame.getSongListHtml(list, mapMyfav))
					})
				}
			},
			getSongListHtml: function(list, mapMyfav) {
				var _tpl_song = ['<li mid="%(playid)">', '<strong class="music_name" title="%(msong)">%(msong)</strong>', '<strong class="singer_name" title="%(msinger)">%(msinger)</strong>',
					'<strong class="play_time">%(time)</strong>', '<div class="list_cp">', '<span class="data">%(songdata)</span>', '<strong class="%(class_like)" title="%(title_like)" name="myfav_%(mmid)" mid="%(mmid)"><span>\u6211\u559c\u6b22</span></strong>', '<strong class="%(class_share)" title="%(title_share)"><span>\u5206\u4eab</span></strong>', '<strong class="%(class_fav)" title="%(title_fav)"><span>\u6536\u85cf</span></strong>', '<strong class="btn_del" title="\u4ece\u5217\u8868\u4e2d\u5220\u9664"><span>\u5220\u9664</span></strong>',
					"</div>", "</li>"
				].join("");
				var html = [];
				MUSIC.object.each(list, function(_music) {
					var music = {};
		...
		var _btnPlayStatus = "g_first_play",
			...
			_bindEvent = function() {
				$D.get("btnplay").onclick = function() {
					...
					if (_status == "g_play") {
						$E.cancelBubble();
						!singleFm.leaving && singleFm.pause();
		...
		return {
			...
			init: function() {
				g_webPlayer.OnSongPlayBegin = function(songinfo, index, total) {
					...
					}); if (songinfo.mtype != "net") $D.get("qrc_ctn").innerHTML =
						"<p>&nbsp;</p><p>\u6b4c\u8bcd\u6b63\u5728\u52a0\u8f7d\u4e2d...</p>";
				...
				g_webPlayer.OnSongPlayEnd = function(songinfo, index, total) {
					...
					if (!!g_msPinnedSite) {
						g_msPinnedSite.paramsConfig.isPlaying =
							false;
						g_msPinnedSite.paramsConfig.isPaused = false;
						g_msPinnedSite.trogglePlayPauseBtn()
					}
					...
					if (_playerOp._isPlaying) {
						...
						if (g_qrcHandlerChn.reqFlag) {
							...
							if (g_qrcHandlerChn.haveqrc && (_innerHtml == "\u6b4c\u8bcd\u6b63\u5728\u52a0\u8f7d\u4e2d..." || _innerHtml == "")) {
								...
								return
							} else if (!g_qrcHandlerChn.haveqrc &&
								...
								if (g_lrcHandlerChn.haveLrc && (_innerHtml == "\u6b4c\u8bcd\u6b63\u5728\u52a0\u8f7d\u4e2d..." || _innerHtml ==
									"")) {
							...
							if (g_qrcHandlerChn.getQrcLineItemByPlayTime(_h5 ? curtime : curtime * 1E3)) {
								...
								if (!!_pNode) {
									...
									if (_h5) {
										...
										if (typeof highlight !== "undefined" && highlight != "") _pNode.innerHTML = _context.replace(highlight, '<span class="current">' +
											highlight + "</span>")
									} else {
							...
							if (g_lrcHandlerChn.getLyricLineItemByPlayTime(g_lrcHandlerChn.ms2playTime(_h5 ? curtime : curtime * 1E3))) {
								var _prePID = g_lrcHandlerChn.preLrcItemIndex == -1 ? "null" :
									g_lrcHandlerChn.preLrcItemIndex,
				...
				g_webPlayer.OnPlaying = function() {
					!singleFm.leaving && singleFm.play();
			...
			renderSong: function(music, mapMyfav) {
				var _tpl_song =
					['<a %(albumhref) target="contentFrame" class="album_pic" title="%(malbum)">', '<img src="%(albumpic)" alt="%(malbum)" onerror="this.src=\'http://imgcache.qq.com/mediastyle/y/img/cover_mine_130.jpg\'" />', "</a>", '<div class="music_info_main">', '<p class="music_name" title="%(msong)"><span>%(msongCut)</span><a onclick="g_topPlayer.singleFm.open();" href="javascript:;" class="icon_radio">\u7535\u53f0</a></p>', '<p class="singer_name" title="%(msinger)">%(msinger)</p>', '<p class="play_date" id="ptime">%(time)</p>',
						'<p class="music_op" style="display:%(display_op);">', '<strong class="%(class_like)" title="%(title_like)" onclick="%(func_like)" name="myfav_%(mmid)" mid="%(mmid)"><span>\u6211\u559c\u6b22</span></strong>', '<strong class="%(class_share)" title="%(title_share)" onclick="%(func_share)"><span>\u5206\u4eab</span></strong>', "</p>", "</div>"
					].join(""),
				...
				if (!music) {
					...
					if (music.mtype == "qqmusic") {
						var _mid = music.malbummid,
							_url = "http://y.qq.com/y/static/album/" + _mid.slice(_mid.length -
								2, _mid.length - 1) + "/" + _mid.slice(_mid.length - 1, _mid.length) + "/" + _mid + ".html";
				...
				if (_gososo != 0 || _down != 1) {
					...
					music.title_share =
						"\u5206\u4eab";
					music.func_share = "g_topPlayer.share(null, g_topPlayer.getCurSongInfo());MUSIC.event.cancelBubble();"
				}
			...
			resetLikeIcon: function() {
				function _do(mapLike) {
					...
					MUSIC.object.each(_es, function(_e) {
			...
			play: function(obj, callback) {
				...
				if (_curTab == "playlist") listFrame.setPlayList(0, _play);
					...
					var uin = g_user.getUin();
					g_mymusicAlbumMod.getList(uin, function(list) {
						for (var i = 0, len = list.length; i < len; i++)
							if (common.isDirOk(list[i])) {
				...
				_setPlayBtnStatus("pause",
					"g_wait");
				pgvClickStat("playone")
			},
			trogglePlay: function() {
				var _e = $D.get("btnplay");
				if (_e.className == "play_bt")
					if (!_playerOp._isFirst) g_webPlayer.startPlayer();
					else _playerOp.play();
			...
			prev: function() {
				...
				_stopTime();
				if (_curPlayDirid == -1) this.play(null, function() {
			...
			like: function(songInfo, self) {
				var uin = g_user.getUin();
				if (uin < 10001) {
					if (/\/y\/static\/login.html/.test(top.window.frames["contentFrame"].location.href)) g_popup.show(1, "\u8bf7\u5148\u767b\u5f55",
						"", 3E3, 200);
				...
				function _func(songinfo) {
					if (self.className == "single_radio_like" || self.className == "btn_like") g_mymusicAlbumMod.Song.like(songinfo.mmid, 3, function() {
						_playerOp.updateSongLike(songinfo.mmid, 1);
						Events.trigger("likeSong", songinfo.mid, 1);
						pgvClickStat("like")
					});
					else g_mymusicAlbumMod.Song.unlike(songinfo.mmid, 3, function() {
			...
			fav: function(obj) {
				...
				function _show() {
					var pos = $D.getPosition(_playerOp._objSong),
						...
						_top = parseInt($D.get("divplaylist").style.top ||
							0),
				...
				if (!_e) {
					...
					g_mymusicAlbumMod.getList(uin, function(list) {
						var _tpl1 = ['<div class="pop_arrow" style="right:14px;"></div>', '<div class="pop_fav_music_frame">', "<h5>\u6536\u85cf\u5230\u6b4c\u5355\uff1a</h5>",
								'<div class="pop_fav_music_main" id="pop_fav_music_main">', '<ul id="favlistdiv">%(dir_list)</ul>', '<div class="pop_scroll" style="display:%(display_scroll);"><span class="pop_scrolling" style="height:60%;top:10%;" id="favscrollbar"></span></div>', "</div>", "</div>"
							].join(""),
						...
						var _es = _e.getElementsByTagName("li");
						MUSIC.object.each(_es, function(e1) {
			...
			add: function(newlist, isPlay) {
				...
				for (var i = newlist.length - 1; i >= 0; i--)
					if (newlist[i].msongurl && (!/(\.m4a)|(\.mp3)|(\&#46;m4a)|(\&#46;mp3)/i.test(newlist[i].msongurl) && !/(share\.weiyun\.qq\.com)|(share\&#46;weiyun\&#46;qq\&#46;com)/i.test(newlist[i].msongurl))) {
						g_popup.show(1,
							"\u6b4c\u66f2\u201c" + newlist[i].msong + "\u201d\u64ad\u653e\u5931\u8d25\uff01", "\u5f88\u62b1\u6b49\uff0cQQ\u97f3\u4e50\u6682\u4e0d\u63d0\u4f9b\u6b64\u683c\u5f0f\u7684\u6b4c\u66f2\u670d\u52a1\u3002", "", 5E3, 360);
						newlist.splice(i, 1)
					}
				_localSong.get(function(list) {
					...
					if (oldlen + addlen > newlen) {
						...
						else showTips("\u6dfb\u52a0\u6b4c\u66f2\u5931\u8d25\uff01",
							"\u8d85\u8fc7\u5bb9\u91cf\u4e0a\u9650\uff0c\u672c\u5730\u7535\u8111\u6700\u591a\u53ef\u4ee5\u7f13\u5b58</br>32k\u7684\u6b4c\u66f2\uff0c\u8bf7\u5220\u9664\u64ad\u653e\u5217\u8868\u5185\u7684\u6b4c\u66f2\u3002", 2);
					...
					if (_curPlayDirid == 0) g_webPlayer.addSong(newlist,
						isPlay);
					else if (isPlay) listFrame.setPlayList(0, function() {
			...
			del: function(obj) {
				...
				} if (dirid == _curPlayDirid)
					if (pos >= 0 && !g_webPlayer.delSong(pos)) {
						_playerOp.renderSong();
						_playerOp._isPlaying = false;
						$C.removeClassName($D.get("divplayer"), "m_player_playing");
						g_webPlayer.stopPlayer()
			...
			setPlayWay: function() {
				_stopTime();
				$E.cancelBubble();
				var _list = ['<strong title="\u968f\u673a\u64ad\u653e" class="unordered_bt" onclick="g_topPlayer.realSetPlayWay(4);"><span>\u968f\u673a\u64ad\u653e</span></strong>', '<strong title="\u5355\u66f2\u5faa\u73af" class="cycle_single_bt" onclick="g_topPlayer.realSetPlayWay(1);"><span>\u5355\u66f2\u5faa\u73af</span></strong>', '<strong title="\u5217\u8868\u5faa\u73af" class="cycle_bt" onclick="g_topPlayer.realSetPlayWay(3);"><span>\u5217\u8868\u5faa\u73af</span></strong>',
						'<strong title="\u987a\u5e8f\u64ad\u653e" class="ordered_bt" onclick="g_topPlayer.realSetPlayWay(2);"><span>\u987a\u5e8f\u64ad\u653e</span></strong>'
					],
					_map = [1, 3, 2, 0],
					_length = _map.length;
	...
	var _volumeMod = function() {
		...
		function init() {
			...
			_update(vol);
			$E.addEvent(_oVolume, "click", function() {
				MUSIC.event.cancelBubble();
	...
	var _playProgressBar = function() {
		...
		function init() {
			...
			$E.addEvent(_bgBar, "click", function() {
				MUSIC.event.cancelBubble();
				MUSIC.event.preventDefault();
			...
			$E.addEvent(_downloadBar, "click", function() {
				MUSIC.event.cancelBubble();
				MUSIC.event.preventDefault();
			...
			$E.addEvent(_oProgress, "click", function() {
				MUSIC.event.cancelBubble();
				MUSIC.event.preventDefault();
				var _pos = $D.getPosition(_bgBar),
					_mousex = $E.mouseX(),
			...
			$E.addEvent(_oOp, "mousedown", function(e) {
				$E.cancelBubble();
				...
				var _startMove = (new Date).getTime(),
					...
					body = document.body;
				$E.addEvent(body, "mousemove", function(e) {
					var now = (new Date).getTime();
					if (now - _startMove < 300) return false;
					_move(e, arr)
				}, arr);
				$E.addEvent(body, "mouseup", function(e) {
			...
			if (!!_timeShow) {
				...
				$E.addEvent(_bgBar.parentNode, "mouseout", function(e) {
		...
		function _update(pos) {
			if (!isNaN(pos) && (pos >= 0 && pos < 101)) {
				...
				if (!_playerOp._isPlaying) g_topPlayer.play(_curPlayElem, function() {
					setTimeout(function() {
		...
		function _toCurLyricLine(lCurPos) {
			...
			if (g_qrcHandlerChn.haveqrc) {
				var _node =
					$D.get("line_" + g_qrcHandlerChn.qrcLineIndex);
				if (!!_node && !!_node.getElementsByTagName)
					if ($C.hasClassName(_node.getElementsByTagName("span")[0], "current")) $C.removeClassName(_node.getElementsByTagName("span")[0], "current");
	...
	function showPlaylist() {
		$E.cancelBubble();
		MUSIC.effect.show("divplayframe", 300, function() {
		...
		if (_isFirstShow) {
			...
			$E.addEvent($D.get("divsongframe"), "click", function() {
				hidePlaylist()
			})
	...
	function hidePlaylist(callback) {
		...
		MUSIC.effect.hide("divplayframe", 300, function() {
	...
	function troggleLrcPannel() {
		...
		if ($C.hasClassName(_btn, "btn_lyrics_enabled")) {
			...
			_heights2 = 0, _top2 = 0, _idx2 = g_lrcHandlerChn.lrcItemIndex, _max = _lrc && _lrc.scrollHeight - 72;
			if (g_qrcHandlerChn.haveqrc) {
	...
	function showDragTips() {
		if (!!top["drag_songinfo"] && top["drag_songinfo"].trim() != "") {
			...
			if (!!_tips) {
				...
				$E.addEvent($D.get("divplayer"), "dragover", function(e) {
					var _x = $E.mouseX(e),
						_y = $E.mouseY(e) - 25;
					$D.setXY(_tips, _x, _y)
				});
				$E.addEvent(frames["contentFrame"].document, "dragover", function(e) {
					var _x = $E.mouseX(e),
						_y = $E.mouseY(e) + 100;
					$D.setXY(_tips, _x, _y)
				});
	...
	var singleFm = function() {
		var $extend = MUSIC.object.extend,
			...
			_singOnPlayBegin = function(status) {
				if (status == "fm") g_webPlayer.OnSongPlayBegin = function(songinfo, index, total) {
					...
					var _curFmSong = $D.getElementsByClassName("current", "li", _FmList)[0],
						...
						_nextEle =
						$D.getNextSibling(_curFmSong);
					if (!singleFm._delBtnStatus)
						if (singleFm.leaving && _curFmSong.getAttribute("mid") == songinfo.mid) singleFm.play();
						else if (_nextEle && _nextEle.getAttribute("mid") == songinfo.mid) singleFm.next();
					else {
						if (!$C.hasClassName(_prevEle, "first")) singleFm.prev()
					} else singleFm._delBtnStatus = false; if (_nextEle && ($D.getNextSibling(_nextEle) && !$D.getNextSibling($D.getNextSibling(_nextEle)))) _model.nextPage();
					var uin = g_user.getUin();
					if (uin < 10001) {
					...
					setTimeout(function() {
						...
						if (!!_e) {
							...
							if (_es.length > index) {
								if (_curPlayElem) {
									$C.removeClassName(_curPlayElem,
										"play_current");
									_curPlayElem = null
								}
								_curPlayElem = _es[index];
			...
			_viewTpl = function(songlist) {
				...
				for (var i = 0, len = songlist.length; i < len; i++) {
					_malbumId =
						songlist[i].malbummid, _imgCache = _malbumId.slice(_malbumId.length - 2, _malbumId.length - 1) + "/" + _malbumId.slice(_malbumId.length - 1, _malbumId.length) + "/" + _malbumId;
			...
			_setFmList = function(dirid, callback) {
				g_webPlayer.setPlayerList(false,
					singleFm.curFMList, 2);
				_curPlayDirid = dirid;
				callback && callback();
				return
			},
			_model = function() {
				var cacheData = {},
					...
					getData = function(sid, cb, nextData) {
						...
						var url =
							"http://s.plcloud.music.qq.com/fcgi-bin/fcg_get_similar_song.fcg?id=" + sid + "&index=" + next + "&num=10&jsonpCallback=MusicJsonFmCallback&_=" + (new Date).valueOf();
						var j = new MUSIC.JSONGetter(url, "singleFM" + sid + next, null, "utf-8", false);
						j.onSuccess = function(data) {
							if (data.code != 0) {
								!nextData && Events.trigger("FMempty", _model, "\u60a8\u8bf7\u6c42\u7684\u6570\u636e\u51fa\u9519\u4e86\uff01");
								return
							}
							...
							if (!_FMdata.retime) {
								retimeStatus = false;
								if (_FMdata.total == 0) {
									!nextData && Events.trigger("FMempty");
								...
								renewConfig(next, sid, false)
							} else retimeStatus = true; if (_FMdata.call_back) {
							...
							if (songsData.length == 0) {
								!nextData &&
									Events.trigger("FMempty", _model, "\u60a8\u8bf7\u6c42\u7684\u6570\u636e\u5217\u8868\u4e3a\u7a7a\uff01");
								renewConfig(0, sid, true);
								return
							}
							var _songlist = [];
							MUSIC.object.each(songsData, function(music) {
								var _cache = g_trackServ.formatMusic(music);
				...
				return {
					...
					report: function(type, data) {
						...
						var url = "http://s.plcloud.music.qq.com/fcgi-bin/fcg_similar_report.fcg?type=" + type + "&qq=" + data.qq + "&songid=" + data.songid + "&oper=" + data.oper +
							"&rid=" + data.rid + "&sid=" + data.sid + "&time=" + (data.time || 0) + _callback + "&g_tk=" + getACSRFToken() + "&playtime=" + (data.playtime || 0) + "&random=" + (new Date).valueOf();
						if (!$D.get("reportImg")) {
			...
			_updateListWidth = function() {
				_FmList.style.width =
					(singleFm.curFMList.length + 1) * 310 + "px"
			},
			...
			_delCurSong = function() {
				...
				var _curSong = $D.getElementsByClassName("current", "li", _FmList)[0],
					_nextSong, _nextNextSong, _mid = _curSong.getAttribute("mid");
				...
				_nextNextSong && (_nextNextSong.className =
					"next");
				_curSong.innerHTML = "";
		...
		return {
			init: function() {
				...
				fmTips.init();
				$E.addEvent(_tabContainer, "click", function(e) {
				...
				$E.addEvent(_FmList, "click", function(e) {
					...
					if (singleFm.leaving) {
						...
						_pos = function() {
							...
							for (var i = 0, len = singleFm.curFMList.length; i < len; i++)
								if (_mid == singleFm.curFMList[i].mid) return i;
							return null
						}();
				...
				$E.addEvent(_FmSongInfo, "click", function(e) {
					$E.cancelBubble();
					var _target = MUSIC.event.getTarget(e);
			...
			next: function() {
				var _dis = $C.hasClassName($D.get("divplayer"), "mini_version") ? -220 : -270,
					_left, _target = $D.getElementsByClassName("next", "li", _FmList)[0],
					_tarNextEle, _tarPreEle, _delEle, _pos = 0;
				...
				if (_dis)
					if (!MUSIC.userAgent.webkit) MUSIC.effect.run(_FmList, {
						...
						_left = parseInt(_FmList.style.marginLeft) +
							_dis;
			...
			updateSongLike: function(mid, status) {
				var _curSong = $D.getElementsByClassName("current",
					"li", _FmList)[0];
				if (_curSong && _curSong.getAttribute("mid") == mid)
					if (status) {
			...
			open: function() {
				...
				if (_songInfo) {
					...
					_stopTime();
					!(_divplayframe.style.display == "block") && showPlaylist();
					_setFm(_songInfo);
					_cutTab("FM");
					_showFm();
	...
	function init() {
		...
		_localSong.get(function(list) {
			...
			$E.addEvent($D.get("divsonglist"), "dblclick", function() {
		...
		try {
			...
			$E.addEvent(_playlist, "dragenter", function(evt) {
				var _e = $E.getTarget(evt).parentNode;
				if (_e.nodeName === "LI" && (!!top["drag_songinfo"] && top["drag_songinfo"].trim() != "")) $C.addClassName(_e, "move_here")
			});
			$E.addEvent(_playlist, "dragover", function(evt) {
				$E.preventDefault()
			});
			$E.addEvent(_playlist, "dragleave", function(evt) {
				var _e = $E.getTarget(evt).parentNode;
				if (_e.nodeName === "LI") $C.removeClassName(_e, "move_here")
			});
			$E.addEvent(_playlist, "drop", function(evt) {
				if (!!top["drag_songinfo"] &&
					top["drag_songinfo"].trim() != "") {
			...
			$E.addEvent($D.get("btnfold"), "dragenter", function(evt) {
				if (!!top["drag_songinfo"] && top["drag_songinfo"].trim() != "") {
					var _e = $E.getTarget(evt);
					if (_e.title == "\u70b9\u51fb\u5c55\u5f00") trogglePlayer()
				}
			});
			$E.addEvent($D.get("divplayer"), "dragenter", function(evt) {
				if (!!top["drag_songinfo"] &&
					top["drag_songinfo"].trim() != "") {
					_stopTime();
					if ($D.get("divplayframe").style.display == "none") showPlaylist()
			...
			$E.addEvent($D.get("divplayer"), "dragover", function(evt) {
				$E.preventDefault()
			});
			$E.addEvent($D.get("divplayer"), "drop", function(evt) {
				if (!!top["drag_songinfo"] && top["drag_songinfo"].trim() != "") {
					var _obj = g_trackServ.formatMusic(top["drag_songinfo"]);
		...
		_iconRadio && _iconRadio.addEventListener("click",
			singleFm.open)
	}
	return {
		init: init,
		...
		encode: function(input) {
			...
			while (i < input.length) {
				...
				output = output + Base64._keyStr.charAt(enc1) +
					Base64._keyStr.charAt(enc2) + Base64._keyStr.charAt(enc3) + Base64._keyStr.charAt(enc4)
			}
			return output
		},
...
MUSIC.channel.qrcHandler = function() {
	...
	return function() {
		...
		_this.init = function(opt) {
			...
			_this.clearQrcInfo();
			_this.loadLyricsBySongId(opt.songMId,
				_this.dealLyricsSucc, _this.dealLyricsFail)
		};
		...
		_this.parseQrcData = function() {
			...
			for (i = 1, _len = _tmpList.length; i < _len; i++) {
				...
				if (_tmpTag.indexOf("al:") != -1) {
					...
					_pointPos = _tmpTag.indexOf(":");
					_this.byBody = _this.qrcJson.by =
						_tmpTag.substring(_pointPos + 1)
		...
		_this.getHighlightWords = function(playTime,
			lineIndex) {
			playTime = playTime || 0;
		...
		_this.scrollLine =
			function(elem) {
				...
				stop = _top == heights;
				if (!stop)
					if (heights - _top >= _line_height *
						2) elem.scrollTop = heights;
					else {
...
(function() {
	...
	window.cancelRequestAnimFrame = function(handle) {
		window.cancelAnimationFrame ? window.cancelAnimationFrame(handle.value) : window.webkitCancelRequestAnimationFrame ? window.webkitCancelRequestAnimationFrame(handle.value) :
			window.mozCancelRequestAnimationFrame ? window.mozCancelRequestAnimationFrame(handle.value) : window.oCancelRequestAnimationFrame ? window.oCancelRequestAnimationFrame(handle.value) : window.msCancelRequestAnimationFrame ? window.msCancelRequestAnimationFrame(handle.value) : window.clearTimeout(handle)
	};
	window.requestTimeout = function(cb, delay) {
		if (!window.requestAnimationFrame && (!window.webkitRequestAnimationFrame && (!window.mozRequestAnimationFrame && (!window.oRequestAnimationFrame && !window.msRequestAnimationFrame)))) return window.setTimeout(cb,
			delay);
...
MUSIC.channel.lrcHandler = function() {
	...
	return function() {
		...
		_this.parseLyricData = function() {
			...
			for (_len = _tmpList.length; i < _len; i++) {
				...
				var _rIndex = _cache.lastIndexOf("]"),
					...
					_tmpTimes = _lSubstr.replace(new RegExp("\\[",
						"g"), "").split("]"),
		...
		_this.getLyricLineItemByTimes = function(minutes, seconds, mSecond) {
			...
			_this.clearLrcItem();
			var _curPlayTime = minutes + ":" + seconds + "." +
				mSecond;
		...
		_this.getLyricLineItemByPlayTime = function(playTime) {
			...
			for (; i < _lrcItemCount - 1; i++) {
				...
				if (_this.compare(_startItem.time, _curPlayTime, "<=") && _this.compare(_curPlayTime,
					_endItem.time, "<")) {
					_lineIndex = i;
		...
		_this.playTime2ms = function(playTime) {
			var _minutes = parseInt(playTime.substring(0, playTime.indexOf(":")), 10) * 60 * 1E3,
				...
				_mSecond = parseInt(playTime.substring(playTime.indexOf(".") +
					1), 10);
		...
		_this.compare = function(pt1, pt2, opt) {
			...
			else if (">" == opt) return _this.playTime2ms(pt1) >
				_this.playTime2ms(pt2);
			else if (">=" == opt) return _this.playTime2ms(pt1) >= _this.playTime2ms(pt2)
		};
		_this.scrollLine = function(elem) {
			...
			var max = parseInt(elem.scrollHeight - 72),
				...
				_times = parseInt(_line_height / 6, 10),
				_off = _line_height % 6;
			while (_idx-- > 0) heights += !!$D.get("line_" +
				_idx) ? $D.get("line_" + _idx).offsetHeight : 24;
			stop = _top == heights;
			if (!stop)
				if (heights - _top >= _line_height * 2) elem.scrollTop = heights;
				else {
					var _diff = heights - _top;
					if (_diff > _line_height) _diff -= _line_height;
					var _t = _diff >= _times ? _times : _off;
...
/*  |xGv00|8ba4447c67f9aa370a2a9c3690ac5f9c */